...
public class BankSetBalance extends Command {
    @Override
    public String[] names() {
        return new String[]{"setbalance"};
    }

    @Override
    public Text description() {
        return Text.of("Установить баланс пользователю");
    }
    ...
    public CommandExecutor executor() {
        return (src, args) -> {
            ...
            bank.set(consumer, amount);
            WFPlugin.log(new LogElement(LogType.SETBALANCE, src.getName(), consumer + ", " + amount));
            src.sendMessage(strings.setBalanceSuccessSrc(consumer, amount));
            Sponge.getServer().getPlayer(consumer).ifPresent(player -> player.sendMessage(strings.setBalanceSuccess(amount)));
    ...
    public String permission() {
        return "wf.admin.bank.setbalance";
    }
}
